...
public class CircularLinkedList<E> implements Iterable<E> {
    ...
    public Node<E> appendLast(E value) {
        ...
        if (last != null) {
            last.next = node;
            node.next = first; // Update last node's next to point back to first
            last = node;
        } else {
            first = node; // If the list is empty, set both first and last to the new node
            last = node;
            node.next = first; // Update new node's next to point back to itself in a circular manner
        }
        length++;
        return node;
    }
    ...
    public Node<E> appendFirst(E value) {
        ...
        if (first == null) {
            ...
            node.next = first;
            first = node;
            last.next = first; // Update last node's next to point back to the new first node
        }
        length++;
        return node;
    }
    ...
    public static void main(String[] args) {
        CircularLinkedList<Integer> anotherList = new CircularLinkedList<>();
        ...
        // Iterate only for two iterations
        int iterations = 0;
